...
import { MessageCircle, MapPin, Share2, Bookmark, Flame, Sparkles, BarChart3, PenSquare, TrendingUp, TrendingDown } from "lucide-react";
...
interface Community {
  id: number; slug: string; name: string; memberCount: number; postCount: number;
}

/* ─── constants ───────────────────────────────────────── */
const POST_META: Record<string, { label: string; color: string; bg: string; dot: string }> = {
  disease_report: { label: "Disease Alert",  color: "text-red-400",    bg: "bg-red-500/10",    dot: "bg-red-500"    },
  market_price:   { label: "Market Update",  color: "text-emerald-400",bg: "bg-emerald-500/10",dot: "bg-emerald-500"},
  weather:        { label: "Climate Alert",  color: "text-blue-400",   bg: "bg-blue-500/10",   dot: "bg-blue-500"   },
  opportunity:    { label: "Opportunity",    color: "text-purple-400", bg: "bg-purple-500/10", dot: "bg-purple-500" },
  question:       { label: "Question",       color: "text-yellow-400", bg: "bg-yellow-500/10", dot: "bg-yellow-500" },
  success_story:  { label: "Success Story",  color: "text-green-400",  bg: "bg-green-500/10",  dot: "bg-green-500"  },
};
...
function sorted(posts: Post[], mode: SortMode) {
  const c = [...posts];
...
function SortBar({ sort, setSort }: { sort: SortMode; setSort:(m:SortMode)=>void }) {
  const btns = [
    { mode:"hot" as SortMode, icon:<Flame className="w-4 h-4"/>, label:"Hot" },
    { mode:"new" as SortMode, icon:<Sparkles className="w-4 h-4"/>, label:"New" },
    { mode:"top" as SortMode, icon:<BarChart3 className="w-4 h-4"/>, label:"Top" },
  ];
  return (
  ...
  return (
    <div className="flex bg-[#1e2025] border border-[#343536] hover:border-[#818384] rounded-lg mb-2 overflow-hidden transition-colors group">
      ...
      <Link href={`/posts/${post.id}`} className="flex-1 min-w-0 p-3 cursor-pointer">
        ...
        {/* Preview */}
        <p className="text-[#818384] text-[12px] leading-relaxed line-clamp-2 mb-3">
          {post.content}
        </p>
    ...
    <div className="bg-[#1e2025] border border-[#343536] rounded-lg overflow-hidden mb-3">
      ...
      <div className="px-4 pb-4">
        ...
        <div className="grid grid-cols-2 gap-2 mb-3">
          <div>
            <div className="font-black text-[#d7dadc] text-[14px]">1,204</div>
            <div className="text-[#818384] text-[10px]">Farmers</div>
          </div>
          <div>
            <div className="font-black text-[#22c55e] text-[14px]">Online</div>
            <div className="text-[#818384] text-[10px]">24 / 7</div>
          </div>
        </div>
        ...
        <button
          onClick={() => nav("/communities")}
          className="w-full mt-2 border border-[#22c55e] text-[#22c55e] text-[12px] font-bold py-2 rounded-full hover:bg-[#22c55e]/10 transition-colors"
        >
          Browse Communities
        </button>
      </div>
...
function MarketSnapshot() {
  return (
    <div className="bg-[#1e2025] border border-[#343536] rounded-lg p-4 mb-3">
      <h3 className="font-bold text-[#d7dadc] text-[11px] uppercase tracking-wider mb-3">
        Market Snapshot
      </h3>
          ...
          <div key={item.crop} className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className={`text-[10px] font-bold flex items-center gap-0.5 ${
                item.up === true ? "text-[#22c55e]" : item.up === false ? "text-[#ef4444]" : "text-[#818384]"
              }`}>
  ...
  return (
    <div className="bg-[#1e2025] border border-[#343536] rounded-lg p-4 mb-3">
      ...
      <Link href="/communities">
        <button className="w-full mt-2 text-[#22c55e] text-[11px] font-bold hover:underline">
...
export default function Feed() {
  ...
  async function handleVote(id: number, direction: "up" | "down") {
    setPosts(prev => prev.map(p =>
      p.id === id ? { ...p, upvotes: direction === "up" ? p.upvotes + 1 : p.upvotes } : p
    ));
    try {
