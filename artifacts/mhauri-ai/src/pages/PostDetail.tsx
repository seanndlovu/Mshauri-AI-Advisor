...
interface Comment {
  ...
  upvotes: number; authorName: string | null; createdAt: string;
}
    ...
    return (
      <div className="h-full flex items-center justify-center bg-[#1a1a1b]">
        <div className="text-center text-[#71767B]">
          ...
          <Link href="/"><button className="text-[#22c55e] hover:underline text-sm font-semibold">Back to feed</button></Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="h-full overflow-y-auto bg-[#1a1a1b]">
      <div className="max-w-2xl mx-auto px-4 py-4">
        ...
        {/* Back */}
        <Link href="/">
          <button className="flex items-center gap-2 text-[#71767B] hover:text-[#E7E9EA] mb-4 transition-colors text-sm">
        ...
        <div className="bg-[#16181C] border border-[#2F3336] rounded-2xl p-5 mb-4">
          ...
          <div className="flex items-center gap-2 mb-3 flex-wrap">
            <span className={`text-[11px] font-bold px-2.5 py-0.5 rounded-full border ${TYPE_COLORS[post.type] ?? TYPE_COLORS.question}`}>
          ...
          <div className="flex items-center gap-4 mt-4 pt-4 border-t border-[#2F3336] flex-wrap">
            <span className="text-[#71767B] text-[13px] font-medium">
              👍 {post.upvotes} helpful
            </span>
            <span className="flex items-center gap-1.5 text-[#71767B] text-[13px]">
              <MessageCircle className="w-4 h-4" />{post.commentCount} {post.commentCount === 1 ? "reply" : "replies"}
            </span>
            <span className="text-[#71767B] text-[13px] ml-auto">u/{post.authorName ?? "anonymous"}</span>
            <span className="flex items-center gap-1 text-[#71767B] text-[13px]">
        ...
        <form onSubmit={submitComment} className="mb-6">
          <div className="flex gap-2">
            <textarea
              ...
              rows={2}
              className="flex-1 bg-[#16181C] border border-[#2F3336] rounded-2xl px-4 py-3 text-[#E7E9EA] text-sm placeholder-[#71767B] focus:outline-none focus:border-[#22c55e] resize-none transition-colors"
            />
            <button type="submit" disabled={submitting || !commentText.trim()}
              className="p-3 bg-[#22c55e] hover:bg-[#16a34a] text-white rounded-2xl disabled:opacity-50 transition-colors self-end">
        ...
        {/* Comments */}
        <div>
          <h2 className="text-[#E7E9EA] font-bold text-[15px] mb-4">
            {post.commentCount === 0 ? "No replies yet" : `${post.commentCount} ${post.commentCount === 1 ? "Reply" : "Replies"}`}
          </h2>

          {comments.length === 0 ? (
            <p className="text-[#71767B] text-sm text-center py-8">Be the first to reply!</p>
          ) : (
            <div className="flex flex-col gap-0">
              {comments.map((comment, i) => (
                <div key={comment.id} className={`py-4 ${i < comments.length - 1 ? "border-b border-[#2F3336]" : ""}`}>
                  <div className="flex items-center gap-2 mb-2">
                    <div className="w-7 h-7 rounded-full bg-[#22c55e]/15 border border-[#22c55e]/20 flex items-center justify-center text-[#22c55e] font-bold text-xs shrink-0">
                      {(comment.authorName ?? "A")[0].toUpperCase()}
                    </div>
                    <span className="text-[#E7E9EA] text-[13px] font-semibold">u/{comment.authorName ?? "anonymous"}</span>
                    <span className="text-[#71767B] text-[11px]">
                      {formatDistanceToNow(new Date(comment.createdAt), { addSuffix: true })}
                    </span>
                  </div>
                  <p className="text-[#E7E9EA] text-[14px] leading-relaxed pl-9">{comment.content}</p>
                </div>
              ))}
            </div>
          )}
